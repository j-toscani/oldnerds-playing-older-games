...
- **Frontend — Verbindung, keine UI-Anbindung (#44):** `apps/website/src/lib/gamedayEvents.ts` hält eine einzige **modul-gescopte** `EventSource` (nie eine pro Komponente/Hook-Aufruf); der erste Subscriber öffnet sie, der letzte schließt sie, ein anderer Gameday-ID-Wunsch tauscht die Verbindung. Ein nativer `addEventListener` je Eintrag aus `GAMEDAY_EVENT_TYPES` fächert an alle Subscriber auf; ein Parse-Fehler landet im `logger`, nicht als throw. Eine Verbindung, die im geschlossenen Zustand (`readyState === CLOSED`) einen Error auslöst — z.B. ein abgelehnter Handshake — wird nach einer gedeckelten Verzögerung neu geöffnet; transiente Aussetzer regelt `EventSource` selbst über sein eingebautes Reconnect. Der Hook `apps/website/src/lib/useGamedayEvents.ts` ist bewusst **zustandslos**: kein `useState`, keine Re-Renders, kein Rückgabewert — nur ein `useEffect`, das (de-)subscribed. **Bewusst noch nicht angebunden:** Die Live-Route `_unauthenticated/gameday.$id.tsx` nutzt den Hook noch nicht; sie zeigt weiterhin nur den Initial-Load aus dem Loader. Kein `router.invalidate()`, kein Re-Rendering, kein Verbindungsstatus in der UI — das folgt in einem eigenen Ticket, das den Hook tatsächlich konsumiert.
