...
- **Frontend (#45):** Hook `apps/website/src/lib/useGamedayEvents.ts` (EventSource, ein nativer `addEventListener` je Handler-Key, Reconnect mit gedeckeltem Backoff) und Live-Route `_unauthenticated/gameday.$id.tsx`, die nach dem Initial-Load ausschließlich per SSE aktualisiert (kein Polling). Der Initial-Load läuft über einen Route-**Loader** (`fetchGameday`, `createServerFn` in `apps/website/src/lib/gameday.ts`) — serverseitig, damit das Session-Cookie an die API weitergereicht werden kann, und weiterhin über den typisierten Hono-RPC-Client (`hc<AppType>`, type-only Ableitung aus `@onog/api`) statt handgeschriebenem `fetch`. Ein `connected`-Event triggert `router.invalidate()`, da verpasste Events nicht nachgeliefert werden. Auth sitzt in der pathless Layout-Route `_unauthenticated.tsx`, nicht in der Seite.
